...
def salvar_dados(categoria, dados):
    if categoria == "alunos":
        cursor.execute("INSERT INTO alunos (nome, idade, cidade) VALUES (?, ?, ?)", dados)
    elif categoria == "professores":
        cursor.execute("INSERT INTO professores (nome, idade, disciplina) VALUES (?, ?, ?)", dados)
    elif categoria == "disciplinas":
        cursor.execute("INSERT INTO disciplinas (nome, turno) VALUES (?, ?)", dados)
...
def consultar_dados(categoria):
    ...
    tree = ttk.Treeview(consulta_janela, columns=[str(i) for i in range(len(cursor.description))], show="headings")
    ...
    for i, desc in enumerate(cursor.description):
        if desc[0] == "id":
            tree.column(str(i), width=30)  # Definindo largura menor para o campo "id"
        else:
            tree.column(str(i), width=70)  # Definindo largura padrão para os demais                                                campos
...
def nova_janela(categoria, atributos):
    ...
    nova_janela.geometry('{}x{}'.format(largura_janela_categoria, altura_janela_categoria))
    ...
    salvar_button = tk.Button(nova_janela, text="Salvar", command=lambda: salvar_dados(categoria, [campo.get() for campo in campos]), width=20)
...
def botao_consulta(categoria):
    return tk.Button(root, text=f"Consultar {categoria.capitalize()}", command=lambda: consultar_dados(categoria), width=20, height=2, font=("Arial", 12))

alunos_button = tk.Button(root, text="Alunos", command=lambda: nova_janela("alunos", ["nome", "idade", "cidade"]), width=20, height=2, font=("Arial", 12))
alunos_button.pack(pady=10)

professores_button = tk.Button(root, text="Professores", command=lambda: nova_janela("professores", ["nome", "idade", "disciplina"]), width=20, height=2, font=("Arial", 12))
professores_button.pack(pady=10)

disciplinas_button = tk.Button(root, text="Disciplinas", command=lambda: nova_janela("disciplinas", ["nome", "turno"]), width=20, height=2, font=("Arial", 12))
